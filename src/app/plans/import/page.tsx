  ...
  for (let si = 0; si < r.sets.length; si++) {
    ...
    for (let ei = 0; ei < set.exercises.length; ei++) {
      ...
      if (typeof ex.title !== "string" || !ex.title)
        return `${path}.sets[${si}].exercises[${ei}].title es requerido`;
      if (typeof ex.preparation_secs !== "number")
        return `${path}.sets[${si}].exercises[${ei}].preparation_secs debe ser un número`;
      if (typeof ex.duration_secs !== "number")
        return `${path}.sets[${si}].exercises[${ei}].duration_secs debe ser un número`;
      if (typeof ex.repetitions !== "number")
        return `${path}.sets[${si}].exercises[${ei}].repetitions debe ser un número`;
    }
  }
...
function validateBundle(
  ...
  for (let di = 0; di < obj.days.length; di++) {
    ...
    if (seenDays.has(day.day_of_week as number)) {
      return {
        ok: false,
        error: `days[${di}].day_of_week ${day.day_of_week} está duplicado`,
      };
    }
    seenDays.add(day.day_of_week as number);

    const routineErr = validateRoutineBundle(
      day.routine,
      `days[${di}].routine`,
    );
...
async function importRoutineBundle(
  ...
  if (idsToVerify.length > 0) {
    ...
    if (fetchError)
      return { error: "Error verificando ejercicios: " + fetchError.message };

    const existingIds = new Set(existing?.map((e) => e.id));
    const missing = idsToVerify.filter((id) => !existingIds.has(id));
    if (missing.length > 0)
      return { error: `Ejercicios no encontrados: ${missing.join(", ")}` };
  }
    ...
    for (let ei = 0; ei < rb.sets[si].exercises.length; ei++) {
      ...
      if (ex.id) {
        ...
        if (createError || !created)
          return {
            error: `Error creando ejercicio "${ex.title}": ${createError?.message}`,
          };
  ...
  for (let si = 0; si < rb.sets.length; si++) {
    ...
    if (setErr || !newSet)
      return { error: `Error creando set "${bs.name}": ${setErr?.message}` };
    ...
    if (exerciseRows.length > 0) {
      ...
      if (linkErr)
        return {
          error: `Error vinculando ejercicios al set "${bs.name}": ${linkErr.message}`,
        };
    }
  ...
  if (routineErr || !routine)
    return {
      error: `Error creando rutina "${rb.routine.name}": ${routineErr?.message}`,
    };
  ...
  if (setEntries.length > 0) {
    ...
    if (linkErr)
      return { error: `Error vinculando sets a rutina: ${linkErr.message}` };
  }
