...
function validateBundle(data: unknown): { ok: true; bundle: RoutineBundle } | { ok: false; error: string } {
  ...
  for (let si = 0; si < obj.sets.length; si++) {
    ...
    if (!Array.isArray(set.exercises) || set.exercises.length === 0) {
      return { ok: false, error: `sets[${si}] debe tener al menos un ejercicio` }
    }
    ...
    for (let ei = 0; ei < set.exercises.length; ei++) {
      ...
      if (typeof ex.title !== 'string' || !ex.title) {
        return { ok: false, error: `sets[${si}].exercises[${ei}].title es requerido` }
      }
      if (typeof ex.preparation_secs !== 'number') {
        return { ok: false, error: `sets[${si}].exercises[${ei}].preparation_secs debe ser un número` }
      }
      if (typeof ex.duration_secs !== 'number') {
        return { ok: false, error: `sets[${si}].exercises[${ei}].duration_secs debe ser un número` }
      }
      if (typeof ex.repetitions !== 'number') {
        return { ok: false, error: `sets[${si}].exercises[${ei}].repetitions debe ser un número` }
      }
    }
  }
  ...
  const handleImport = async () => {
    ...
    try {
        ...
        for (let ei = 0; ei < bundle.sets[si].exercises.length; ei++) {
          ...
          if (ex.id) {
            ...
            if (createError || !created) {
              setError(`Error creando ejercicio "${ex.title}": ${createError?.message}`)
              return
            }
      ...
      for (let si = 0; si < bundle.sets.length; si++) {
        ...
        if (linkError) {
          setError(`Error vinculando ejercicios al set "${bs.name}": ${linkError.message}`)
          return
        }
