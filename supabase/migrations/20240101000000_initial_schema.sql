-- ─────────────────────────────────────────────────────────────────────────────
-- Rutinea — Initial Schema
-- Run this in your Supabase SQL Editor (or via supabase db push)
-- ─────────────────────────────────────────────────────────────────────────────

-- Enable pg_crypto for slug generation
...
create policy "routines: public read" on public.routines for select using (is_public = true);

-- sets: accessible if the user owns the parent routine OR the routine is public
create policy "sets: own all" on public.sets for all
  using (
    exists (
...
create policy "sets: public read" on public.sets for select
  using (
    exists (
      select 1 from public.routines r
      where r.id = routine_id and r.is_public = true
    )
  );
